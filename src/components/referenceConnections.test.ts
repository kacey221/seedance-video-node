...
test('treats connected generated images with src as usable references', () => {
  const generatedImage = createLayer({
    id: 'gen-1',
    name: 'Generated Image',
    src: 'data:image/png;base64,generated',
  });
  const emptyImage = createLayer({
    id: 'gen-2',
    name: 'Ungenerated Image',
    src: '',
  });
  const videoNode = createLayer({
    id: 'video-1',
    name: 'Video Node',
    type: 'video',
    parentIds: ['gen-1', 'gen-2'],
  });

  const connected = getConnectedReferences([generatedImage, emptyImage, videoNode], videoNode);
