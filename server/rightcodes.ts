...
export type RightCodesDetailLevel = '1K' | '2K' | '4K';
...
export interface RightCodesImageRequestInput {
  ...
  model?: string;
  detailLevel?: RightCodesDetailLevel;
}
...
function mapRightCodesModel(model: string | undefined): string {
  const selectedModel = model || 'gpt-image-2';
  return RIGHTCODES_MODEL_ALIASES[selectedModel] || selectedModel;
}
...
function mapNanoBananaOutputSize(
  model: string | undefined,
  aspectRatio: SupportedAspectRatio,
  detailLevel: RightCodesDetailLevel | undefined,
): string {
  const baseSize = mapAspectRatioToRightCodesSize(aspectRatio);
  if (
    !['nano-banana-2', 'nano-banana-pro'].includes(model || '') ||
    !detailLevel ||
    detailLevel === '1K' ||
    baseSize === 'auto'
  ) {
  ...
  const multiplier = detailLevel === '4K' ? 4 : 2;
...
export function buildRightCodesImageRequest(input: RightCodesImageRequestInput): RightCodesImageRequestBody {
  ...
  const body: RightCodesImageRequestBody = {
    model: mapRightCodesModel(input.model),
    prompt: combinedPrompt,
    size: mapNanoBananaOutputSize(
      input.model,
...
export function normalizeRightCodesUpstreamError(status: number, details: string): string {
  ...
  if (/^\s*(?:<!doctype\s+html|<html\b)/i.test(details)) {
    return `Right Codes returned an unexpected server error (${status}). Please try again.`;
  }
...
export async function postRightCodesGenerationRequest(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    try {
      response = await fetchImpl('https://www.rightapi.ai/draw/v1/images/generations', {
        method: 'POST',
        headers: {
