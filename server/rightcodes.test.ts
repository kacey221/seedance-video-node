...
test('maps Image 2.5 aliases to RightAPI GPT Image model identifiers', () => {
  const standard = buildRightCodesImageRequest({
    model: 'image-2.5',
    prompt: 'cinematic portrait',
  });
  const flare = buildRightCodesImageRequest({
    model: 'image-2.5-flare',
    prompt: 'cinematic portrait',
  });
  const sunburst = buildRightCodesImageRequest({
...
test('maps nano-banana-2 and nano-banana-pro detail levels to larger output sizes', () => {
  const nanoBanana2 = buildRightCodesImageRequest({
    model: 'nano-banana-2',
    prompt: 'cinematic portrait',
    aspectRatio: '3:4',
    detailLevel: '2K',
  });
  const nanoBananaPro = buildRightCodesImageRequest({
    model: 'nano-banana-pro',
  ...
  assert.equal(nanoBanana2.size, '2048x3072');
  assert.equal(nanoBananaPro.size, '6144x4096');
});

test('keeps the existing 1K size when no detail level is provided', () => {
  const request = buildRightCodesImageRequest({
    model: 'nano-banana-pro',
    prompt: 'square icon',
    aspectRatio: '1:1',
...
test('posts image generations to the current Right API domain', async () => {
  let requestedUrl = '';

  await postRightCodesGenerationRequest({
    apiKey: 'test-key',
  ...
  assert.equal(requestedUrl, 'https://www.rightapi.ai/draw/v1/images/generations');
});
...
test('normalizes noisy upstream error bodies into friendly messages', () => {
  ...
  assert.equal(
    normalizeRightCodesUpstreamError(
      502,
      '<!DOCTYPE html><!--[if lt IE 7]><html class="no-js ie6 oldie"><![endif]--><head></head></html>',
    ),
    'Right Codes returned an unexpected server error (502). Please try again.',
  );
});
